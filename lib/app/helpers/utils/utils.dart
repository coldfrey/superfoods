import 'package:superfoods/app/helpers/extensions/date_time_extention.dart';
...
class Utils {
  ...
  // extracts the error message from a Firebase error that is user-friendly
  static String extractErrorMessage(String firebaseError) {
    // Pattern to match everything after "] "
    RegExp regExp = RegExp(r'\] (.*)');
    // Find the first match
    RegExpMatch? match = regExp.firstMatch(firebaseError);
    // If there's a match, return the matched group; otherwise, return the original error
    return match?.group(1) ?? firebaseError;
  }
}
